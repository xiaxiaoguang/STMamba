...
def TrainSTG_Mamba(train_dataloader, valid_dataloader, A, K=3, num_epochs=1, mamba_features=307):
    ...
    inputs, labels = next(iter(train_dataloader)) # 32,12,207 是输入数据
...
def TestSTG_Mamba(kfgn_mamba, test_dataloader, max_speed):
    ...
    #std_MAE measures the consistency & stability of the model's performance across different test sets or iterations. Usually if (std_MAE/MSE)<=10%., means the trained model is good.
