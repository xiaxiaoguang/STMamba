...
def TrainiMamba(train_dataloader, valid_dataloader, test_dataloader, 
                   num_epochs=1, model_args=None,
                   max_speed = 1):
    inputs, labels = next(iter(train_dataloader))
    [batch_size, step_size, fea_size] = inputs.size()
    rnd = torch.rand(1).item()
    folder_path = f'checkpoints/{rnd}'
    ...
    expand = 2
    n_layers = 2
    # STmamba = imamba(num_len, pred_len,embed_dim,
    #                 d_state, d_conv, expand, n_layers).cuda()
    with open('checkpoints/imamba5/mamba_9.pkl', 'rb') as file:
        STmamba = pickle.load(file)
    STmamba.cuda()
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_dataloader:
            ...
            if trained_number % interval == 0:
                ...
                with open(folder_path+"/record2.txt",'a') as f:
        ...
        TestiMamba(STmamba,test_dataloader,max_speed,epoch, folder_path)

    return STmamba, [losses_train, losses_interval_train, losses_valid, losses_interval_valid]


def TestiMamba(STmamba, test_dataloader, max_speed,epoch , folder_path):
    ...
    with open(folder_path + f"/record2.txt",'a') as f:
        print('Tested: MAE: {}, std_MAE: {}, MAPE: {}, MSE: {}, RMSE: {}, VAR: {}'.format(MAE_, std_MAE_, MAPE_, MSE_, RMSE_, VAR_),file=f)

    with open(folder_path + f'/mamba_{epoch%10}.pkl', 'wb') as file:
